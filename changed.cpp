//transformation en dame dans boite mousepressed event a la fin
        ...
        for(i=0; i<taille; i++)
        {
            if(game->tab_damier[0*taille+i]==this)
            {
        ...
        for(i=0; i<taille; i++)
        {
            if(game->tab_damier[(taille-1)*taille+i]==this)
            {
...
void generateur::remplirListes()
{
    ...
    if(!this->dame) //on remplit les liste pour pion normal
    {
        //si le pion est noir
    ...
    if(this->dame) //on rempli les listes pour dames
    {
        //des choses a prendre sur la diagonal avt gauche
